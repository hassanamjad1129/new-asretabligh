...
@section('content')
    <div class="card">
        ...
        <div class="card-block">
            <form action="{{ route('admin.productProperties.update',[$product,$productProperty]) }}"
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" class="form-control"
                           value="{{old('name')?old('name'):$productProperty->name}}"/>
                </div>
                <div class="col-md-6">
                    <label for="">نوع مشخصه</label>
                    <select class="form-control" name="type" style="padding: 0.1rem"
                            id="info" disabled>
                        <option value="1" {{($productProperty->type=='input')?'selected':''}}>توضیح</option>
                        <option value="2" {{($productProperty->type=='selectable')?'selected':''}}>تک انتخابی</option>
                    </select>
                </div>
                <div class="col-md-12">
                    <label for=""> توضیحات مشخصه</label>
                    <input type="text" name="description" class="form-control"
                           value="{{old('description')?old('description'):$productProperty->description}}"/>
                </div>
                @if($productProperty->type=='selectable')
                    <div id="selectAble">
                        <div class="col-md-12">
                            ...
                            <div id="answer">
                                <?php $i = 1;?>
                                @foreach($productAnswers as $key=>$productAnswer)
                                    <div class="clearfix"></div>
                                    <div class="card">
                                        <div class="card-block">
                                            <div class="col-md-6">
                                                <label for="">مقدار</label>
                                                <input type="text" name="answer_{{$productAnswer->id}}"
                                                       class="form-control"
                                                       value="{{$productAnswer->name}}"/>
                                            ...
                                            <div class="col-md-3">
                                                <button type="button" class="btn btn-sm btn-primary" id="response"
                                                        style="margin-top: 1rem">{{($productAnswer->picture)?"تصویر فعلی":"معین نشده"}}</button>
                                                <div class="clearfix"></div>
                                                <img src="{{ route('admin.ProductAnswer',[$productAnswer]) }}"
                                                     style="width: 50%; margin:0.7rem 0"
                                                     onerror="this.style.display='none';">
                                            </div>
                                            <div class="col-md-3">
                                                <a class="deleteBTN btn btn-danger btn-sm"
                                                   href="{{route('admin.destroyValue',[$product,$productProperty,$productAnswer])}}"
